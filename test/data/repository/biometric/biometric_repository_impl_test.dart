...
void main() {
  ...
  setUp(() {
    ...
    repository = BiometricRepositoryImpl(mockLocalAuth, mockLocalDataSource);
  });
  ...
  group('BiometricRepositoryImpl', () {
    ...
    group('isBiometricSupported', () {
      ...
      test('should return Right(true) when canCheckBiometrics is false but device is supported', () async {
        // arrange
      ...
      test('should return Left(UnexpectedFailure) when an exception occurs', () async {
        // arrange
        when(() => mockLocalAuth.canCheckBiometrics).thenThrow(tException);
    ...
    group('authenticate', () {
      test('should return Right(true) when authentication is successful', () async {
        // arrange
    ...
    group('saveSecretKey', () {
      test('should return Right(unit) when data source succeeds', () async {
        // arrange
        when(() => mockLocalDataSource.saveSecretKey(any()))
            .thenAnswer((_) async => {});
      ...
      test('should return Left(CacheFailure) when data source throws CacheException', () async {
        // arrange
        when(() => mockLocalDataSource.saveSecretKey(any())).thenThrow(tCacheException);
      ...
      test('should return Left(UnexpectedFailure) when data source throws unexpected exception', () async {
        // arrange
        when(() => mockLocalDataSource.saveSecretKey(any())).thenThrow(tException);
    ...
    group('getSecretKey', () {
      test('should return Right(Some(key)) when data source returns a key', () async {
        // arrange
        when(() => mockLocalDataSource.getSecretKey())
      ...
      test('should return Left(CacheFailure) when data source throws CacheException', () async {
        // arrange
        when(() => mockLocalDataSource.getSecretKey()).thenThrow(tCacheException);
    ...
    group('deleteSecretKey', () {
      test('should return Right(unit) when data source succeeds', () async {
        // arrange
        when(() => mockLocalDataSource.deleteSecretKey())
            .thenAnswer((_) async => {});
      ...
      test('should return Left(CacheFailure) when data source throws CacheException', () async {
        // arrange
        when(() => mockLocalDataSource.deleteSecretKey()).thenThrow(tCacheException);
    ...
    group('setBiometricEnabled', () {
      ...
      test('should return Left(CacheFailure) when data source throws CacheException', () async {
        // arrange
        when(() => mockLocalDataSource.setBiometricEnabled(any())).thenThrow(tCacheException);
    ...
    group('isBiometricEnabled', () {
      ...
      test('should return Left(CacheFailure) when data source throws CacheException', () {
        // arrange
        when(() => mockLocalDataSource.isBiometricEnabled()).thenThrow(tCacheException);
